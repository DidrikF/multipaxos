...
func setTestingHook(fd *EvtFailureDetector) <-chan bool {
	...
	fd.testingHook = func() { //overwriting testingHook function on the failure detector
...
func TestSendReplyToHeartbeatRequest(t *testing.T) {
	acc := NewAccumulator()                                               //SuspectRestorer
	hbOut := make(chan Heartbeat, 16)                                     //hbSend argument, used by failure detector to send heartbeats
	fd := NewEvtFailureDetector(ourID, clusterOfThree, acc, delta, hbOut) //create failure detector
	done := setTestingHook(fd)                                            //
...
func TestTimeoutProcedure(t *testing.T) {
	for i, test := range timeoutTests {
		...
		if !reflect.DeepEqual(test.wantPostSuspected, fd.suspected) {
			...
			t.Errorf("%+v, %+v", test.wantPostSuspected, fd.suspected)
