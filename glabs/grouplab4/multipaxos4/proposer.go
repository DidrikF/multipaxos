...
// its id.
//		_						-1
func NewProposer(id, nrOfNodes, adu int, ld detector4.LeaderDetector, prepareOut chan<- Prepare, acceptOut chan<- Accept) *Proposer {
	proposer := &Proposer{
		Id:     id,
		quorum: (nrOfNodes / 2) + 1,
		n:      nrOfNodes,

		catchupFactor:     Round(1),
		crnd:              Round(id),
		adu:               SlotID(adu),
		aduAtRestoreEvent: SlotID(adu),
...
func (p *Proposer) Start() {
	...
	recoverMsgs := p.ld.SubscribeToRecoverEvents()
	go func() {
		...
		for {
			...
			case prm := <-p.promiseIn:
				...
				fmt.Printf("#### PROPOSER: Got quorum of Promises! nextSlot: %v, p.adu: %v\n", p.nextSlot, p.adu)
				p.nextSlot = p.adu + 1 // adu is a slotID!
				p.acceptsOut.Init()    //reset/init the list
				p.catchupFactor = Round(1)
			...
			case cval := <-p.cvalIn:
				//fmt.Printf("Proposer: client value in - proposer id: %v - leader: %v - phaseOneDone: %v\n", p.Id, p.leader, p.phaseOneDone)
				if p.Id != p.leader {
					continue
				}
				...
				cvalIsUnique := true
				for i < p.requestsIn.Len() {
					if i == 0 {
						element = p.requestsIn.Front()
					} else {
						element = element.Next()
					}
					value := element.Value.(Value)

					if value.ClientID == cval.ClientID && value.ClientSeq == cval.ClientSeq {
						cvalIsUnique = false
						break
					}
					i++
			...
			case <-p.incDcd: //IncrementAllDecidedUpTo, this is what will trigger sendAccept() most of the time (in normal operation)
				fmt.Printf("##### PROPOSER: handlng increment of adu. p.adu: %v, p.aduAtRestoreEvent: %v, p.leader: %v, p.phaseOneDone: %v\n", p.adu, p.aduAtRestoreEvent, p.leader, p.phaseOneDone)
				p.adu++
				if p.aduAtRestoreEvent <= p.adu {
					p.restoreMode = false
				}
				if p.Id != p.leader {
					continue
			...
			case restoredNode := <-recoverMsgs: //restoredNode :=
				fmt.Printf("##### Proposer: Node %v was recovered and all decided slots are learned again\n", restoredNode)
				if p.Id == p.leader && restoredNode <= p.Id {
					p.aduAtRestoreEvent = p.adu
					p.restoreMode = true
					p.adu = -1
					p.nextSlot = 0
					p.startPhaseOne()
...
func (p *Proposer) IncrementAllDecidedUpToInRestoreMode() {
	fmt.Printf("#### PROPOSER: RestoreMode is true, incrementing adu: %v\n", p.adu)
	if p.restoreMode == true {
			...
			for rsIndex, registedSlot := range registeredSlots {
				...
				if slotFromPromise.ID == registedSlot.ID {
					if slotFromPromise.Vrnd > registedSlot.Vrnd { // I need to remember PromiseSlot.ID
						registeredSlots = append(registeredSlots[:rsIndex], append([]PromiseSlot{slotFromPromise}, registeredSlots[rsIndex+1:]...)...) // not uniform agreement
					}
...
func (p *Proposer) increaseCrnd() {
	p.crnd = p.crnd + Round(p.n)*p.catchupFactor
	p.catchupFactor += Round(1)
}
...
func (p *Proposer) startPhaseOne() {
	...
	//fmt.Printf("Proposer: phase one started. Crnd: %v\n", p.crnd)
}
