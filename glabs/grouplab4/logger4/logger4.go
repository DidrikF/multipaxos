...
type Logger struct {
	...
	WG            *sync.WaitGroup
	webClientChan chan LogMessage
}

type LogMessage struct {
	O string `json:"origin"`
	D string `json:"destination"`
	S string `json:"severity"`
...
func CreateAndStartLogger(nodeId int, logDir string, systemLog string, networkLog string, severity string, loud bool, save bool, netonly bool, wg *sync.WaitGroup, webClientChan chan LogMessage) (*Logger, error) {
	logger := &Logger{
		Loud:          loud,
		NodeId:        nodeId,
		NetOnly:       netonly,
...
func (log *Logger) Log(msg LogMessage) { //make safe for concurrent calls
	...
	//logMessage := timeStamp + "  -  " + strings.ToUpper(msg.S) + "  -  #" + msg.O + ": " + msg.M

	logMessageToClients := msg
	messageStringToClients := timeStamp + "  -  " + strings.ToUpper(msg.S) + "  -  " + msg.M
	logMessageToClients.M = messageStringToClients

	if log.Loud == true {
		log.webClientChan <- logMessageToClients
	}
...
func (log *Logger) Stop() {
	close(log.logChan) // problem, ensure that no one will write to logger before calling the Stop() method.
	log.stopChan <- struct{}{}
}
